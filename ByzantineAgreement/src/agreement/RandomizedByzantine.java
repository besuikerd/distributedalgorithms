...
public class RandomizedByzantine extends AbstractProcess<IMessage> implements Runnable {
	...
	public boolean doWork(boolean v, int f) {
		...
		while (true) {
			...
			List<NotificationMessage> notificationMessages = await(NotificationMessage.class, r, n - f);
			...
			List<ProposalMessage> proposalMessages = await(ProposalMessage.class, r, n - f);
			...
			if ((received0 + received1) > f) {
				...
				if (oldV != v) {
					log("[" + r + "] Switched from v = " + oldV + " to " + v);
				}
			}
			r++;
			log("Increased round to "+ r);
		}
	}
